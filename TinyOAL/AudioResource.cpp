...
{
  // This MUST be called BEFORE reaching ~AudioResource! Otherwise, the virtual functions will have been destructed and the resulting calls from stopping the audio will try to call a pure function.
  if(_activelist)
  {
    ...
    for(Audio* cur = _inactivelist; cur != 0; cur = cur->next) cur->Invalidate();
  }
}
...
AudioResource* AudioResource::Create(FILE* file, unsigned int datalength, TINYOAL_FLAG flags, unsigned char filetype, uint64_t loop)
{
  return AudioResource::_fcreate(file, datalength, flags | TINYOAL_COPYINTOMEMORY, filetype, bss::StrF("%p", file), loop);
}

AudioResource* AudioResource::Create(const void* data, unsigned int datalength, TINYOAL_FLAG flags, unsigned char filetype, uint64_t loop)
{
  if(!data || datalength < 8) //bad file pointer
  {
    if(!data) TINYOAL_LOG(2, "NULL pointer passed in to AudioResource::Create()");
    else TINYOAL_LOG(2, "datalength equal to 0 in AudioResource::Create()");
    return 0;
  }
...
AudioResource* AudioResource::_fcreate(FILE* file, unsigned int datalength, TINYOAL_FLAG flags, unsigned char filetype, const char* path, uint64_t loop)
{
  if(!file || datalength < 8) //bad file pointer
  {
    if(!file) TINYOAL_LOG(2, "NULL pointer passed in to AudioResource::Create()");
    else TINYOAL_LOG(2, "datalength equal to 0 in AudioResource::Create()");
    return 0;
  }
