...
AudioResourceFLAC::AudioResourceFLAC(void* data, unsigned int datalength, TINYOAL_FLAG flags, uint64_t loop) : AudioResource(data, datalength, flags, TINYOAL_FILETYPE_FLAC, loop)
{
...
AudioResourceFLAC::~AudioResourceFLAC()
{
  _destruct();
}
void* AudioResourceFLAC::OpenStream()
{
...
uint64_t AudioResourceFLAC::Tell(void* stream)
{
  return ((DatStreamEx*)stream)->cursample;
}
void AudioResourceFLAC::_cberror(const FLAC__StreamDecoder *decoder, FLAC__StreamDecoderErrorStatus status, void *client_data)
{
  switch(status)
  {
...
FLAC__StreamDecoderWriteStatus AudioResourceFLAC::_cbwrite(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
...
size_t AudioResourceFLAC::__flac_fseek_offset=0; // Special seek function that properly deals with external FILE* handles we can get in ToWave
FLAC__StreamDecoderSeekStatus AudioResourceFLAC::_cbfseekoffset(const FLAC__StreamDecoder *decoder, FLAC__uint64 absolute_byte_offset, void *client_data)
{
...
{
  if(p) new(p) AudioResourceFLAC(data, datalength, flags, loop);
  return sizeof(AudioResourceFLAC);
}
bool AudioResourceFLAC::ScanHeader(const char* fileheader)
{
...
FLAC__StreamDecoderWriteStatus AudioResourceFLAC::_cbemptywrite(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{ // This function exists so we can yank the metadata out of the stream without actually putting it anywhere.
  return FLAC__STREAM_DECODER_WRITE_STATUS_CONTINUE;
}

FLAC__StreamDecoderReadStatus AudioResourceFLAC::_cbread(const FLAC__StreamDecoder *decoder, FLAC__byte buffer[], size_t *bytes, void *client_data)
{
  if(*bytes > 0) {
...
FLAC__StreamDecoderSeekStatus AudioResourceFLAC::_cbseek(const FLAC__StreamDecoder *decoder, FLAC__uint64 absolute_byte_offset, void *client_data)
{
...
FLAC__StreamDecoderTellStatus AudioResourceFLAC::_cbtell(const FLAC__StreamDecoder *decoder, FLAC__uint64 *absolute_byte_offset, void *client_data)
{
  long pos;
...
FLAC__StreamDecoderLengthStatus AudioResourceFLAC::_cblength(const FLAC__StreamDecoder *decoder, FLAC__uint64 *stream_length, void *client_data)
{
...
FLAC__StreamDecoderReadStatus AudioResourceFLAC::_cbfread(const FLAC__StreamDecoder *decoder, FLAC__byte buffer[], size_t *bytes, void *client_data)
{
  if(*bytes > 0) {
...
FLAC__StreamDecoderSeekStatus AudioResourceFLAC::_cbfseek(const FLAC__StreamDecoder *decoder, FLAC__uint64 absolute_byte_offset, void *client_data)
{
...
FLAC__StreamDecoderTellStatus AudioResourceFLAC::_cbftell(const FLAC__StreamDecoder *decoder, FLAC__uint64 *absolute_byte_offset, void *client_data)
{
  long pos;
