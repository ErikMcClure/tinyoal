...
AudioResourceMP3::~AudioResourceMP3() { _destruct(); }
...
bool AudioResourceMP3::Reset(void* stream) { return Skip(stream, 0); }
...
std::pair<void*, unsigned int> AudioResourceMP3::ToWave(void* data, unsigned int datalength, TINYOAL_FLAG flags)
{
  ...
  if(err != MPG123_OK)
    return fnabort(h, "Failed to open mpg handle");

  err       = fn->fn_mpgScan(h);
  off_t len = fn->fn_mpgLength(h);
  if(err != MPG123_OK || len < 0)
    return fnabort(h, "Failed to scan mp3");

  long freq;
  int channels, enc;
  err = fn->fn_mpgGetFormat(h, &freq, &channels, &enc);
  if(!err)
    err = fn->fn_mpgFormatNone(h);
  if(!err)
    err = fn->fn_mpgFormat(h, freq, channels, enc);
  if(err != 0)
...
ssize_t AudioResourceMP3::cb_datread(void* stream, void* dst, size_t n) { return dat_read_func(dst, 1, n, stream); }
...
ssize_t AudioResourceMP3::cb_fileread(void* stream, void* dst, size_t n) { return fread(dst, 1, n, (FILE*)stream); }
